...
// declaração que permite que você acesse nomes 
// do namespace std sem precisar digitar o prefixo std::
using namespace std;
...
private:
    ...
    char Endereco[41];
    char Telefone[13];
    // membros publicos, acessados por qualquer parte do codigo 
    // que acessa o objeto da classe
public:
    Pessoa();          // Construtor default definido pelo programador
    Pessoa(Pessoa &P); // Construtor de cópia

    // getters e setters são métodos usados para acessar e modificar 
    // atributos privados de uma classe.
    int getRG();
    int getCPF();
